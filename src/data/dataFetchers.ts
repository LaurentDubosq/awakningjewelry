...
export const getSiteMenu = (): UseFetchWithStateReturn<SiteMenuItem[]> => {
  const result: UseFetchWithStateReturn<SiteMenuItem[]> =
    useFetchWithState(siteMenuUrl);
  return result;
};

export const getHeroSlides = (): UseFetchWithStateReturn<HeroSlideType[]> => {
  const result: UseFetchWithStateReturn<HeroSlideType[]> =
    useFetchWithState(heroSlidesURL);
  return result;
};

export const getStatementMission =
  (): UseFetchWithStateReturn<StatementBanner> => {
    const result: UseFetchWithStateReturn<StatementBanner> =
      useFetchWithState(statementMissionURL);
    return result;
  };

export const getCollectionsByGender = (): UseFetchWithStateReturn<
  Collection[]
> => {
  const result: UseFetchWithStateReturn<Collection[]> = useFetchWithState(
    collectionsByGenderURL
  );
  return result;
};
...
export const getPromotions = (): UseFetchWithStateReturn<ProductSummary[]> => {
  const result: UseFetchWithStateReturn<ProductSummary[]> =
    useFetchWithState(promotionsURL);
  return result;
};
