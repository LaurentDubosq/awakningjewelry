...
export const useStatementMissionResultStore = defineStore(
  "StatementMissionResult",
  () => {
    // States
    const statementMissionResult: Ref<
      undefined | UseFetchWithStateReturn<StatementBanner>
    > = ref();

    // Computeds
    const statementMissionData: ComputedRef<StatementBanner | undefined> =
      computed(() => unref(statementMissionResult.value?.data));

    const statementMissionFetchStatus: ComputedRef<FetchStatus | undefined> =
      computed(() => unref(statementMissionResult.value?.status));
