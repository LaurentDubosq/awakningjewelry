...
export const useCollectionsByGenderResultStore = defineStore(
  "CollectionsByGenderResult",
  () => {
    // States
    const collectionsByGenderResult: Ref<
      undefined | UseFetchWithStateReturn<Collection[]>
    > = ref();

    // Computeds
    const collectionsByGenderData: ComputedRef<Collection[] | undefined> =
      computed(() => unref(collectionsByGenderResult.value?.data));

    const collectionsByGenderFetchStatus: ComputedRef<FetchStatus | undefined> =
      computed(() => unref(collectionsByGenderResult.value?.status));
